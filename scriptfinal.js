...
function initializeCards() {
  ...
  if (Math.abs(offsetX) > threshold) {
    ...
    if (frontCard) {
      ...
      // Use frontCard data for the swipe action
      const data = {
        title: frontCard.querySelector('.title').innerText,
        year: frontCard.querySelector('.year').innerText,
        genres: frontCard.querySelector('.genres').innerText,
        url: frontCard.querySelector('.card-image').src,
      };

      // Determine the swipe direction
      const isSwipeRight = offsetX > 0;

      if (isSwipeRight) {
        // Swipe right, store in 'favorite'
        console.log(data);
        sendToPHP('storeFavorite.php', data);
      } else {
        // Swipe left, store in 'history'
        console.log(data);
        sendToPHP('storeHistoryM.php', data);
      }
    }
  }
...
function sendToPHP(script, data) {
  // Make an AJAX request to the PHP script
  axios.post(`http://localhost/Mobile/${script}`, data)
    .then(response => {
